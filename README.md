...
The album uses an original crescent-and-cloud illustration, a gold underline, warm paper tones, and a curved golden trail through the growth ribbon. Selected months and the current ribbon photo receive a gold accent; desktop photos gently zoom on hover. A fixed background of slowly drifting gold, sage and blush light with six faint stars follows the whole page, behind photos and controls. It pauses in hidden tabs and is removed for reduced-motion preferences. These details use inline SVG and CSS with no additional image requests or dependencies.
