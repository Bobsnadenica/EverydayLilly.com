...
The growth ribbon also glides automatically at a gentle pace, rests at either end, and reverses without duplicating photos. Warm light, tiny golden stars and lightly tilted photo cards give it a storybook feel. The wheel plays automatically without a play/stop control. Hover and focus do not latch it into a paused state; manual navigation briefly pauses motion and then resumes automatically. Motion is suspended while a photo is open, during upload selection, and when the ribbon or tab is hidden. Reduced-motion preferences disable autoplay and decorative animation. The animation makes no API calls and reuses the existing cached previews.
